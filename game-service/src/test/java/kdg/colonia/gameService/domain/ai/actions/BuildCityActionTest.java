...
class BuildCityActionTest {
    ...
    public void placeCity() {
        CostObject city = infoService.getBuildingCostInfo().stream().filter(c -> c.getObjName().equals("City")).findFirst().get();
        ...
        Action action = new BuildCityAction(currentPlayer.getPlayerId(), new Coordinate(0,0,0, Direction.TOP), gameLogicService);
        action.performAction(game);

        assertEquals(1, game.getBoard().getSettlements().stream().filter(Settlement::isCity).count());
        assertEquals(1, game.getBoard().getSettlements().stream().filter(Settlement::isCity).findFirst().get().getPlayerId());
