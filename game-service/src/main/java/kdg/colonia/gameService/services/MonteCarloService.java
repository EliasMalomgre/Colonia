...
import kdg.colonia.gameService.domain.ai.*;
import kdg.colonia.gameService.domain.ai.actions.Action;
import kdg.colonia.gameService.domain.ai.actions.ChanceAction;
import kdg.colonia.gameService.domain.ai.actions.GroupNode;
...
public class MonteCarloService {
    ...
    public Game findNextMove(Game game) {
        return findNextState(game, aiService.getCurrentPlayer(game)).getFirst();
    }

    public Pair<Game, Action> findNextState(Game game, int playerId) {
        Tree tree = new Tree();
        int simulations = 0;
        ...
        if (aiConfig.isGarbageCollectStartSimulation()) System.gc();
        ...
        if (simStruct.getTime() < 0) {
            return Pair.of(simStruct.getGame(), simStruct.getAction());
        }
        ...
        do {
            ...
            if (aiConfig.isGarbageCollectMidSimulation()&&simulations % aiConfig.getGarbageCollectFrequency() == 0) {
                System.gc();
                try {
                    Thread.sleep(aiConfig.getWaitTimeMidSimulation());
        ...
        if (aiConfig.isGarbageCollectEndSimulation()) System.gc();

        return Pair.of(new Game(winnerNode.getState().getGame()), winnerNode.getState().getAction());
    }
